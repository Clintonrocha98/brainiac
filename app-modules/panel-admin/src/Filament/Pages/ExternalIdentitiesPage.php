...
class ExternalIdentitiesPage extends Page
{
    ...
    public function getProviderCards(): array
    {
        ...
        return collect(IdentityProvider::cases())
            ->map(static function (IdentityProvider $provider) use ($identities): array {
    ...
    public function disconnectAction(): Action
    {
        return Action::make('disconnect')
            ...
            ->action(static function (array $arguments): void {
                $identityId = $arguments['identity_id'] ?? null;

                if (!is_string($identityId) && !is_int($identityId)) {
                    return;
                }

                $identity = ExternalIdentity::query()->findOrFail($identityId);
